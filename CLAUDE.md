...
## Current Deployment Status (as of 2026-08-04)

**Live at https://innonet.work** — DigitalOcean droplet, Docker Compose + Caddy stack. First successful production deploy completed 2026-08-04 after a long debugging session. Full runbook: `docs/DEPLOYMENT.md`; automation scripts: `scripts/`.

**Droplet access:** `ssh deploy@<droplet-ip>` (IP resolves from `innonet.work`'s A record), app lives at `/opt/innonet`. Check current status:
```bash
cd /opt/innonet
docker compose -f docker-compose.prod.yml --env-file .env.production ps
```

**What got fixed to reach this first successful deploy** (commits `1103d70`..`65a2254` on `main`):
- `alembic/env.py` was building its engine from `alembic.ini`'s hardcoded `sqlalchemy.url`, which had a real committed database password, instead of `Settings.get_database_url()` — fixed, and the leaked password removed from the tracked file. **That password may still be in git history** on this public repo (`Mystery312/innonet-public`) — treat it as compromised if reused anywhere. A full history scrub (`git filter-repo` + force-push) was deliberately not done and is still outstanding if wanted.
- `init_db()` in `backend/src/database/postgres.py` called `Base.metadata.create_all()` on every app startup in every environment, racing Alembic migrations. Now skipped when `ENVIRONMENT=production`.
- `docker-compose.prod.yml` no longer auto-mounts `backend/init-db.sql` as a Postgres init script (it's meant to run manually after migrations, not automatically before the schema exists).
- The 7-file migration chain had drifted badly from the current models (missing columns, renamed columns/tables, a missing `password_reset_tokens` table, `profile_embeddings.embedding` using the wrong type). Replaced with one autogenerated, dependency-order-corrected migration: `backend/alembic/versions/ef27d6f70177_consolidated_initial_schema.py`. Verified via a full upgrade→downgrade→upgrade round-trip plus `alembic check` showing zero drift against the models.
- `alembic/env.py` never imported `src.discover.models`, so `DiscoverSwipe` silently never registered with `Base.metadata` — fixed.
- The droplet only had ~1GB RAM at one point during this session, well under the documented 4GB minimum (Neo4j alone wants 1-2GB heap) — root cause of several mid-session container crash loops. **Confirm it's still resized to at least 4GB/2vCPU.**

**Known outstanding items (not blocking, deliberately left as-is):**
- **Email verification**: `SENDGRID_API_KEY` not configured in `.env.production` — verification emails just print to backend container logs instead of sending. Get one now with `docker compose -f docker-compose.prod.yml --env-file .env.production logs backend | grep -A3 "Email to"`. To fix properly: sign up for SendGrid, set `SENDGRID_API_KEY` + `SENDGRID_FROM_EMAIL` in `.env.production`, restart the backend container.
- **Google/Microsoft OAuth sign-in**: `GOOGLE_CLIENT_ID`/`SECRET` and `MICROSOFT_CLIENT_ID`/`SECRET` not configured — buttons redirect with an empty `client_id` and get rejected by the provider. Callback URLs needed when registering OAuth apps:
  - Google: `https://innonet.work/api/v1/auth/oauth/google/callback`
  - Microsoft: `https://innonet.work/api/v1/auth/oauth/microsoft/callback`
- **GitHub Actions auto-deploy** not wired up yet — add `VPS_HOST`, `VPS_USER=deploy`, `VPS_SSH_KEY`, `VPS_DEPLOY_PATH=/opt/innonet` as repo secrets so pushes to `main` auto-deploy instead of requiring manual SSH.
- `scripts/verify-deployment.sh` has two known bugs: a shell integer-comparison error around line 169 (cosmetic, doesn't affect the pass/fail result), and its Redis check runs `redis-cli ping` without `-a "$REDIS_PASSWORD"`, so it always reports "not responding" even when Redis is healthy. Not yet fixed.
