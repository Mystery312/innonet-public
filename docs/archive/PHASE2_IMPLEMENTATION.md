...
**Status:** ✅ SUPERSEDED — Phase 3 Complete (2026-04-28)
**Phase 2 Date:** 2026-04-19
**Phase 3 Date:** 2026-04-28

> **Note:** Phase 3 (plaintext column removal) completed on 2026-04-28. The feature flags, dual-write logic, `read_encrypted_field()` helper, and `*_ct` column naming described in this document no longer exist in the codebase. All PII columns now use `EncryptedString`/`EncryptedText`/`EncryptedJSON` SQLAlchemy TypeDecorators directly. This document is preserved as a historical record of the Phase 1→2 transition.

---

## Overview

Phase 2 switched all application reads from plaintext columns to encrypted columns (`*_ct`). Phase 3 subsequently dropped all plaintext columns and renamed `*_ct` to canonical names, with TypeDecorators handling transparent encrypt-on-write / decrypt-on-read.

**Key Change (Phase 2):** Data that was being written to encrypted columns but read from plaintext was switched to **read from encrypted columns and decrypted on-the-fly**.

**Phase 3 superseded this:** The `USE_ENCRYPTED_COLUMNS` feature flag, `read_encrypted_field()`, and all `*_ct` references are removed. See `SECURITY_ROADMAP.md` for current state.
